...
## 4. Challenges in Scaling Unsupervised Training
...
## 5. Challenges of MLE-Based Unsupervised Training

### A. MLE Through the KL Lens
MLE minimizes:

$$
\text{KL}(P_{\text{data}} \| P_\theta)
$$

**Implication:**
The model is forced to cover all sequences in the data, even incoherent or noisy ones.

This leads to:
* Repetition
* Generic completions
* Improper confidence calibration
* Memorization of rare or strange patterns
* Strong dependence on sampling strategy
* Low-entropy token distributions → overconfidence

### B. What MLE Does Not Do
MLE provides:
* **No semantics**
* **No task awareness**
* **No explicit objective for usefulness**
* **No reward or utility signal**

The model only learns to predict the next token, not to perform tasks or reason in a human-aligned way.

### C. Entropy and Confidence
Token-level entropy:
...
* Low entropy = high confidence → often too high.
* **Key question:** Do we want LLMs to be low-entropy (precise) or high-entropy (diverse)?

### D. Variational Inference vs. MLE
Recall ELBO:
...
Maximizing ELBO increases entropy of $q(z)$.
But maximizing likelihood does **NOT** increase entropy of model predictions—in fact, it often reduces it.

## 6. Fixes / Alternatives to Pure MLE

### 1. Explicit Entropy Regularization

$$
\theta^* = \arg\max_\theta \mathbb{E}[\log P_\theta(x)] + \lambda\sum_t H[P_\theta(x_t \mid x_{<t})]
$$

Prevents collapse by encouraging diversity.
...
Reduces overconfidence in token prediction.

### 3. Contrastive Losses
Encourage separability and richer representations:
* Contrastive predictive coding
* Noise contrastive estimation

### 4. Scheduled Sampling
Injects noise or replaces ground-truth tokens with sampled tokens during training.

### 5. Utility / Preference-Based Losses
